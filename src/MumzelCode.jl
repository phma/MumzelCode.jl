...
function intCodeword(int::Integer)
  @assert typemax(int)>=2^36
  cw=UInt8[]
  for i in 0:5
    push!(cw,(int>>(i*7))&0x7f)
  end
  cw[6]&=1
  Codeword(cw)
end
...
# 43434 10 5^5=3125	31250 125*250 100-16f 00-df 25088=112*224
# 33435 20 5^4*3=1875   37500 150*250 170-205 00-df 33600=150*224
# 43425 60 5^3*3^2=1125 67500 270*250 000-0ff 00-df 57344=256*224
# 33525 30 5^2*3^3=675  20250  81*250 000-205 e0-ff 16576=518*32=74*224
...
function makeEncodeTable33435()
  table=OffsetVector(fill(0x0000,150),-1)
  for i in 0:149
    table[i]=(i÷75)<<8|(i÷15%5)<<5|(i÷3%5)<<2|(i%3+1)
  end
  table
end
...
function makeEncodeTable33525()
  table=OffsetVector(fill(0x00,81),-1)
  for i in 0:80
    table[i]=(i÷27)<<6|(i÷9%3+1)<<4|(i÷3%3+1)<<2|(i%3+1)
    # The high trit does not get 1 added because it goes to the permutation,
    # not to a letter.
  end
  table
end
